...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled || menuOpen ? 'bg-white/95 backdrop-blur-xl border-b border-cotton-warm shadow-sm' : 'bg-transparent'
    }`}>
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-14 md:h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <img src="/icon.png" alt="UMKM Ciangsana" className="h-8 md:h-9 w-auto" />
          <span className="font-bold text-sm md:text-base text-noir hidden sm:block">UMKM Ciangsana</span>
        </Link>
              ...
              return (
                <Link key={l.href} href={l.href}
                  className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-base font-medium transition-all ${
                    isActive ? 'bg-cherry/10 text-cherry' : 'text-noir-soft hover:bg-cotton-warm'
                  }`}>
                  <span className={`${isActive ? 'text-cherry' : 'text-cool-gray'}`}>
                    <Icon name={l.icon} size={20} />
                  </span>
                  {l.label}
                  {isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-cherry" />}
                </Link>
              )
            })}
            <a href={generateWhatsAppLink(ADMIN_WHATSAPP, WHATSAPP_JOIN_MESSAGE)}
