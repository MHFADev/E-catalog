...
// Menjadi source of truth foto; kolom products.images tetap dipakai saat
// product_images kosong (fallback JSON lama) supaya tampilan tidak pecah.
const getProductImagesMap = async function loadProductImages() {
  ...
  try {
    ...
    // DB gagal -> biarkan map kosong (fallback products.images)
  }
  return map;
};
...
const _loadProducts = async function loadProductsData() {
  ...
  try {
    ...
    const fromDb = data.map((row) => withRating(withImages(mapProduct(row))));
