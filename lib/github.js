...
// Batas dihitung pada isi base64, bukan ukuran file mentah. Nilai sengaja jauh
// di bawah batas request deployment agar upload browser tidak bergantung pada
// mekanisme Server Actions.
const MAX_BROWSER_BASE64_LENGTH = 1_100_000;
...
function uploadFailure(status, upstreamMessage = "") {
  if (status === 401 || status === 403) {
    return "GitHub menolak izin upload. Minta admin memeriksa akses token GitHub untuk repository gambar.";
  }
  if (status === 404) {
    return "Repository atau jalur penyimpanan gambar belum ditemukan di server.";
  }
  if (status === 409) {
    return "GitHub sedang memproses upload lain. Sistem akan mencoba ulang otomatis.";
  }
  if (status === 413) {
    return "Gambar terlalu besar untuk dikirim. Pilih gambar lain lalu coba lagi.";
  }
  if (status === 422) {
  ...
  if (status === 429 || status >= 500) {
    return "Layanan GitHub sedang sibuk. Sistem akan mencoba ulang otomatis.";
  }
...
function getUploadConfig() {
  ...
  if (!owner || !repo || !token) {
    throw new Error(
      "Upload gambar belum siap di server. Hubungi admin untuk melengkapi konfigurasi penyimpanan gambar.",
    );
  }
...
async function putImage({
  ...
  extension = "webp",
  folder = "",
  maxBase64Length = MAX_BROWSER_BASE64_LENGTH,
}) {
...
// Dipanggil hanya oleh route handler sesudah sesi pengguna diverifikasi.
