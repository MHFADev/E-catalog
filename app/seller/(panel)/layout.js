      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-10">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl md:text-2xl font-bold tracking-tight">
              Area <span className="text-cherry">Penjual</span>
            </h1>
          ...
          <div className="flex items-center gap-2">
            <Link href="/" className="text-xs md:text-sm text-warm-gray hover:text-cherry px-3 py-2">
              Lihat Situs →
            </Link>
            <SellerLogoutButton />
          </div>
        ...
        {!approved ? (
          account?.status === "pending" || account?.status === "rejected" ? (
            <div className="bg-white rounded-2xl md:rounded-3xl p-6 md:p-10 border border-cotton-warm text-center max-w-lg mx-auto">
          ...
          <>
            <nav className="flex gap-2 overflow-x-auto pb-2 mb-6">
              <Link href="/seller" className="shrink-0 px-4 py-2 text-xs md:text-sm font-semibold rounded-full bg-white border border-cotton-warm text-noir-soft hover:border-cherry hover:text-cherry transition-all">
                Dashboard
              </Link>
              <Link href="/seller/products" className="shrink-0 px-4 py-2 text-xs md:text-sm font-semibold rounded-full bg-white border border-cotton-warm text-noir-soft hover:border-cherry hover:text-cherry transition-all">
                Produk Saya
              </Link>
            </nav>
            {children}
