      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/"
              className="text-xs md:text-sm text-warm-gray hover:text-cherry px-3 py-2"
            >
              Lihat Situs →
            </Link>
            <LogoutButton />
        ...
        <nav className="flex gap-2 overflow-x-auto pb-2 mb-6">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="shrink-0 px-4 py-2 text-xs md:text-sm font-semibold rounded-full bg-white border border-cotton-warm text-noir-soft hover:border-cherry hover:text-cherry transition-all"
            >
              {l.label}
            </Link>
          ))}
