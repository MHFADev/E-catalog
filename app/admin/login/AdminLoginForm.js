...
export default function AdminLoginForm() {
  ...
  // [FIX SESI HILANG] Setelah login berhasil (cookie `admin_auth` sudah ter-set
  // oleh server action), pindah ke /admin dengan FULL PAGE LOAD via
  // `window.location.href`. Ini memastikan browser mengirim cookie sesi pada
  // request berikutnya sehingga layout admin bisa memvalidasi dan admin tidak
  // "harus login ulang" setelah navigasi.
  useEffect(() => {
    if (state?.ok) {
